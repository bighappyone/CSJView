Pod::Spec.new do |s|
  ...
  # 资源文件需要从Ads-CN Pod中加载，请在使用此SDK的项目中安装Ads-CN Pod
  ...
  # 依赖声明：需要在目标项目中安装Ads-CN Pod以提供资源文件
  # 注意：这里不直接依赖Ads-CN，因为使用二进制分发时应该由应用项目管理依赖
  # 但需要在Podfile中同时添加：
  #   pod 'CSJView', :podspec => 'path/to/CSJView.binary.podspec'
  #   pod 'Ads-CN'  # 必需：提供资源文件
